  ...
  providers: [
    {
      ...
      useFactory: (config: ConfigService) => {
        ...
        console.log(`[User-Service] Connecting to Redis at ${redisHost}:${redisPort}`);
        ...
        const client = new Redis({
          ...
          retryStrategy: (times) => {
            if (times > 3) {
              console.error('❌ [User-Service] Redis max retries reached. Stopping reconnection.');
        ...
        client.connect().catch((err) => {
          console.error('❌ [User-Service] Redis connection failed:', err.message);
        });
